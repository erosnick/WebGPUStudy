...
fn randomInUnitDisk() -> vec3f {
    var p: vec3f;
    while (true) {
        p = vec3(randomInRange(-1.0, 1.0), randomInRange(-1.0, 1.0), 0.0);
        if (dot(p, p) >= 1.0) {
            continue;
        }
        break;
...
fn sphereCenter(time: f32) -> vec3f {
...
fn main(@builtin(global_invocation_id) globalInvocationID: vec3u) {
    ...
    let lookFrom = camera.lookFrom.xyz;

    // vertical field-of-view in degrees
    let lensRadius = camera.aperture / 2.0;
    var focusDistance = camera.focusDistance;

    var viewportHeight = camera.viewportHeight;
    var viewportWidth = camera.viewportWidth;
    ...
    let origin = lookFrom;
    var horizontal = focusDistance * viewportWidth * u;
    var vertical = focusDistance * viewportHeight * v;
    var lowerLeftCorner = origin - horizontal / 2.0 - vertical / 2.0 - focusDistance * w;
    ...
    for (var i: u32 = 0; i < sceneData.samplePerPixels; i++) {
        var ray: Ray;
        
        let s = (f32(screenPosition.x) + rand()) / f32(screenSize.x - 1);
        let t = (f32(screenSize.y - screenPosition.y) + rand()) / f32(screenSize.y - 1);
        ...
        ray.time = randomInRange(camera.time0, camera.time1);
