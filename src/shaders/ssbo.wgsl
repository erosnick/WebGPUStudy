...
@group(0) @binding(2) var<uniform> eye: vec4f;
@group(0) @binding(3) var<uniform> lightPosition: vec3f;
@group(0) @binding(4) var<storage, read> objects: ObjectData;
...
@vertex
fn vs_main(@builtin(instance_index) instanceId: u32,
           @location(0) position: vec3f, 
           @location(1) normal: vec3f, 
           @location(2) color: vec3f,
           @location(3) texcoord: vec2f) -> VertexOutput {
  var output : VertexOutput;
  output.Position = projectionMatrix * viewMatrix * objects.model[instanceId] * vec4f(position, 1.0);
  output.worldPosition = (objects.model[instanceId] * vec4f(position, 1.0)).xyz;
  output.normal = (objects.model[instanceId] * vec4(normal, 0.0)).xyz;
  output.color = color;
...
fn fs_main(@location(0) worldPosition: vec3f, 
  ...
  var specular = pow(max(dot(h, n), 0.0), 320.0);
