...
@group(0) @binding(5) var albedo: texture_2d<f32>;
...
fn fs_main(@location(0) worldPosition : vec3f, 
  ...
  var albedo = textureSample(albedo, textureSampler, texcoord).rgb;

  return vec4f(ambient + albedo * diffuse + vec3f(specular), 1.0);
