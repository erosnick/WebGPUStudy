...
import shader from "./shaders/ssbo.wgsl?raw"
...
async function initPipeline(device: GPUDevice, format: GPUTextureFormat) {
	...
	modelBuffer = device.createBuffer({
		size: MatrixSize * 2, //行数 * 列数 * BYTES_PER_ELEMENT
	...
	let modelMatrix0 = mat4.translation([-1.0, 0.0, 0.0])
	let modelMatrix1 = mat4.translation([1.0, 0.0, 0.0])

	device.queue.writeBuffer(modelBuffer, 0, modelMatrix0)
	device.queue.writeBuffer(modelBuffer, MatrixSize, modelMatrix1)
...
function draw(
	...
	// 绘图，3 个顶点
	passEncoder.drawIndexed(pipelineObj.cube.indexCount, 2, 0, 0, 0)
